...
with right_col:
    ...
    if st.session_state[markers_state] is not None:
        fig.add_trace(
            go.Scattermapbox(
                mode="markers",
                marker=go.scattermapbox.Marker(size=24, color="red"),
    ...
    fig.update_layout(
        ...
        mapbox=dict(
            style="open-street-map",  # Mapbox hatake OpenStreetMap use ho raha hai
...
if st.session_state[last_openai_run_state] is not None:
    st.write(f"Assistant run id: {get_run_id()}")
    st.write(f"Assistant thread id: {get_thread_id()}")
