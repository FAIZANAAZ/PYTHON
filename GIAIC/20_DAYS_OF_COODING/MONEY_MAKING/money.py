...
def fetch_side_hustle():
    try:
        ...
        response = requests.get("http://127.0.0.1:8000/side_hustles?apiKey=secret")
...
def fetch_money_quote():
    try:
        ...
        response = requests.get("http://127.0.0.1:8000/money_quotes?apiKey=secret")
        if response.status_code == 200:  # If request successful
...
if st.button("Get Inspired"):  # When user clicks button
 ...
 #abhi wo error hi dega kioky esa url hm direct nhi le skty bina deploy ke  
 
 
#  api ko loali active krny keliye hm terminal me os folder tc jaengy jha api bnai thi 
# run cd
# run cd folder name
# run .venv\Scripts\activate
# run fastapi dev api.py (api.py yani file name )
# click http://127.0.0.1:8000/docs in terminal